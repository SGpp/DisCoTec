...
int simft::Sim_FT_MPI_Test(simft::Sim_FT_MPI_Request *request, int *flag, simft::Sim_FT_MPI_Status *status){
	...
	if((*request)->Request_Type == 1){ //P2P
		...
		if((*request)->f_comm->comm_revoked ) {
			...
			*flag = 1;
			if(status != SIM_FT_MPI_STATUS_IGNORE)
							status->MPI_ERROR = MPI_ERR_REVOKED;
			return MPI_ERR_REVOKED;
		}else if((*request)->P2P_Stats.p2p_rank == MPI_ANY_SOURCE && (*request)->f_comm->dead_set.size() > 0 && (*request)->f_comm->Ack_failed_processes == false){
			//ULFM standard behavior for MPI_ANY_SOURCE. it is recommended to revoke the comm after this error occurs!!!
			if((*request)->c_request!=MPI_REQUEST_NULL) simft::Sim_FT_MPI_Request_free(request);
			*flag = 1;
			if(status != SIM_FT_MPI_STATUS_IGNORE)
				status->MPI_ERROR = MPI_ERR_PROC_FAILED_PENDING;
			...
			*flag = 1;
			if(status != SIM_FT_MPI_STATUS_IGNORE)
				status->MPI_ERROR = MPI_ERR_PROC_FAILED;
		...
		if(!(*request)->completed){
			...
			if(status != SIM_FT_MPI_STATUS_IGNORE){
				...
				//status->cancelled = status->c_status.cancelled; //this only exists in MPICH, not in Open MPI => better not assignt it
			}
			if(CFlag){
		...
		if((*request)->completed){ //operation is completed, continue to check for alive/status messages
			int Alive_Flag = 0;
			MPI_Status Alive_Status;
			MPI_Iprobe((*request)->P2P_Stats.p2p_rank, (*request)->P2P_Stats.tag, (*request)->f_comm->c_comm_copy_p2p, &Alive_Flag, &Alive_Status);
			if(Alive_Flag){
				*flag = 1;
				(*request)->completed = false;
				MPI_Status recv_status;
				char Alive_Status_Recv;
				MPI_Recv(&Alive_Status_Recv, 1, MPI_CHAR, Alive_Status.MPI_SOURCE, (*request)->P2P_Stats.tag, (*request)->f_comm->c_comm_copy_p2p, &recv_status);
				...
				if((*request)->P2P_Stats.p2p_rank == MPI_ANY_SOURCE){
					...
					MPI_Isend(&Alive_Status_Send, 1, MPI_CHAR, Alive_Status.MPI_SOURCE, (*request)->P2P_Stats.tag, (*request)->f_comm->c_comm_copy_p2p, &Send_Request);
