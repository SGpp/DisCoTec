...
//   manager.recomputeOptimumCoefficients(prob_name, faultsID, redistributeFaultsID,
//                                        recomputeFaultsID);
//   // // timestep does not need to be updated in gene but maybe in other applications
//   // for (auto id : redistributeFaultsID) {
//   //   SelalibTask* tmp = static_cast<SelalibTask*>(manager.getTask(id));
//   //   tmp->setCombiStep(i + 1);  // adjust combistep for fault criterion!
//   // }

//   // for (auto id : recomputeFaultsID) {
//   //   SelalibTask* tmp = static_cast<SelalibTask*>(manager.getTask(id));
//   //   tmp->setCombiStep(i + 1);  // i+1 as decideToKill is not executed during recompute and therfore
//   //                              // combistep is not increased
//   // }
...
//   } else {
//     // if process groups could be restored reinitialize restored process group (keep the original
//     // tasks)
...
//     manager.recompute(recomputeFaultsID, failedRecovery,
//                       groupFaults);  // toDO handle faults in recompute
//   }
...
int main(int argc, char** argv) {
  ...
  combitime =
      cfg.get<combigrid::real>("application.combitime");  // combitime between combinations (can be
                                                          // used instead of fixed stepnumber)
  nsteps =
   ...
   * negative value mean that we want a fault distribution according to the Weibul distr.-> -1000
   * means e.g. Weibul with lambda = 1000
   */
  ...
  for (size_t i = 0; i < levels.size(); i++) {
    ...
    if (faultsInfo.numFaults_ != 0 &&
        dynamic_cast<LearningLoadModel*>(loadmodel.get()) != nullptr) {
    ...
    Task* t =
        new SelalibTask(dim, levels[i], boundary, coeffs[i], loadmodel.get(), path, dt, combitime,
                        nsteps, p, faultCrit);
    tasks.push_back(t);
  ...
  CombiParameters params(dim, lmin, lmax, boundary, levels, coeffs, hierarchizationDims, taskIDs,
                         ncombi, numGrids, reduceCombinationDimsLmin, reduceCombinationDimsLmax);
  ...
  std::cout
      << "initialization took "
      << std::chrono::duration_cast<std::chrono::microseconds>(after_init_time - init_time).count()
      << "us " << std::endl;
