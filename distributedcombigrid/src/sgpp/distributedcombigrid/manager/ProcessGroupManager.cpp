...
bool ProcessGroupManager::runNewTask(Task* t) {
  return storeTaskReferenceAndSendTaskToProcessGroup(t, RUN_NEWTASK);
}
...
void ProcessGroupManager::startBestExpansion(){
  sendSignalAndReceive(BEST_EXPANSION);
}
...
void ProcessGroupManager::sendTaskToProc(const std::map<int, int>& taskToProc){
  sendSignalToProcessGroup(TASK_TO_PROC);
...
void ProcessGroupManager::addExpansion(const LevelVector& expansion){
  ...
  sendSignalToProcessGroup(ADD_EXPANSION);
