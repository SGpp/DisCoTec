...
int simft::Sim_FT_MPI_Comm_split(simft::Sim_FT_MPI_Comm f_comm, int color, int key, simft::Sim_FT_MPI_Comm *f_newcomm){
	...
	if(MPI_SUCCESS == Ret){
		simft::Sim_FT_Initialize_new_comm((f_newcomm), true); //TODO: if "reroot" implemented, set to false
	}else{
		delete (*f_newcomm);
