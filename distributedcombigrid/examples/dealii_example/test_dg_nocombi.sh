...
log='log'
#create ctparam
echo "\n DG without combination now ncombi=100 und dt=0.01">> l2_error_combi.dat

for ((lmax=5;lmax<=6;lmax++)); do
    echo "lmax=$lmax"
    ...
    for ((lmin=1;lmin<=lmax;lmin ++)); do
        ...
        sh  ./run2.sh
        fileOne="out/FE_DGQ/csmi_$lmin,$lmin,${lmin}_ma_$lmax,$lmax,${lmax}_ev_$lmax,$lmax,${lmax}_combi_false.dat5"
...
echo "\n DG without combination now ncombi=10 und dt=0.1">> l2_error_combi.dat
