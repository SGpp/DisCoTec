...
int simft::Sim_FT_MPI_Iallreduce(void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype,
   MPI_Op op, simft::Sim_FT_MPI_Comm f_comm, simft::Sim_FT_MPI_Request *request){
	...
	NewCollective.Datatype = simft::Datatype_to_Dtype(datatype); //"transform" MPI datatype to int, so we can store and send it
	NewCollective.op = simft::MOp_to_Op( op ); //"transform" the MPI operation to int

	//create nem custom struct object, set the object in NewCollective and attach NewCollective to Recent_Icollectives
	...
	return MPI_Iallreduce(sendbuf, recvbuf, count, datatype,
			   op, f_comm->c_comm, &(*request)->c_request);
}
#endif
