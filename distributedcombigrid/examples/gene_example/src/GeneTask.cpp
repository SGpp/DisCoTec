...
GeneTask::GeneTask(DimType dim, LevelVector& l, std::vector<bool>& boundary, real coeff,
                   LoadModel* loadModel, std::string& path, real dt, size_t nsteps)
    : Task(dim, l, boundary, coeff, loadModel), path_(path), dt_(dt), nsteps_(nsteps) {
...
void GeneTask::run(CommunicatorType& lcomm) {
  // change dir to wdir
...
void GeneTask::writeLocalCheckpoint(GeneComplex* data, size_t size, std::vector<size_t>& sizes,
...
void GeneTask::getFullGrid(FullGrid<complex>& fg, RankType localRootID, CommunicatorType& lcomm) {
  ...
  if (localID == localRootID) recvdata.resize(static_cast<size_t>(lsize) * lcp.getSize());

  MPI_Gather(senddata, static_cast<int>(lcp.getSize()), MPI_DOUBLE_COMPLEX, &recvdata[0],
             static_cast<int>(lcp.getSize()), MPI_DOUBLE_COMPLEX, localRootID, lcomm);

  // gather bounds
  sendbounds.assign(lcp.getBounds().begin(), lcp.getBounds().end());

  if (localID == localRootID) recvbounds.resize(lsize * sendbounds.size());

  MPI_Gather(&sendbounds[0], static_cast<int>(sendbounds.size()), MPI_INT, &recvbounds[0],
             static_cast<int>(sendbounds.size()), MPI_INT, localRootID, lcomm);
  ...
  if (localID == localRootID) {
    ...
    // create GeneGrid
    GeneGrid global(boost::extents[globalSize[0]][globalSize[1]][globalSize[2]][globalSize[3]]
                                  [globalSize[4]][globalSize[5]]);
    ...
    for (size_t r = 0; r < static_cast<size_t>(lsize); ++r) {
      ...
      GeneGridRef local(data, boost::extents[ln_spec][lnw0][lnv0][lnz0][lnj0][lni0]);

      // fill global array with data
      // set view to global array
      typedef GeneGrid::index_range range;
      GeneGrid::array_view<6>::type lview =
          global[boost::indices[range(bounds[0], bounds[1])][range(bounds[2], bounds[3])]
                               [range(bounds[4], bounds[5])][range(bounds[6], bounds[7])]
                               [range(bounds[8], bounds[9])][range(bounds[10], bounds[11])]];
          ...
          for (index l = 0; l < lnv0; ++l)
            for (index k = 0; k < lnz0; ++k)
              for (index j = 0; j < lnj0; ++j)
                for (index i = 0; i < lni0; ++i) lview[n][m][l][k][j][i] = local[n][m][l][k][j][i];
    }
...
void GeneTask::setLocalCheckpoint(FullGrid<complex>& fg, CommunicatorType lcomm,
  ...
  if (localID == localRootID) {
    std::vector<int> levels(this->getLevelVector().begin(), this->getLevelVector().end());
    ...
    const std::vector<size_t>& globalSize = this->getLocalCheckpoint().getSizes();
    GeneGrid global_tmp(boost::extents[globalSize[0]][globalSize[1]][globalSize[2]][globalSize[3]]
                                      [globalSize[4]][globalSize[5]]);
    ...
    for (size_t r = 0; r < static_cast<size_t>(lsize); ++r) {
      ...
      typedef GeneGrid::index_range range;
      GeneGrid::array_view<6>::type lview =
          global_tmp[boost::indices[range(bounds[0], bounds[1])][range(bounds[2], bounds[3])]
                                   [range(bounds[4], bounds[5])][range(bounds[6], bounds[7])]
                                   [range(bounds[8], bounds[9])][range(bounds[10], bounds[11])]];
  ...
  MPI_Scatter(&recvdata[0], static_cast<int>(lcp_tmp_data.size()), MPI_DOUBLE_COMPLEX,
              &lcp_tmp_data[0], static_cast<int>(lcp_tmp_data.size()), MPI_DOUBLE_COMPLEX,
              localRootID, lcomm);
  ...
  GeneGridRef local(ldata, boost::extents[ln_spec][lnw0][lnv0][lnz0][lnj0][lni0]);
  GeneGridRef local_tmp(ldata_tmp, boost::extents[ln_spec][lnw0][lnv0][lnz0][lnj0][lni0]);
...
void GeneTask::saveCheckpoint(const FullGrid<complex>& fg, const char* filename) {
