...
int simft::Sim_FT_MPI_Send(void *buf, int count, MPI_Datatype type, int dest,
             int tag, simft::Sim_FT_MPI_Comm f_comm)
{
    ...
    while(!leaveloop){
		...
		/*
		 * If comm is revoked, a recent "Sim_FT_Perform_background_operations" could already have responded the
		 * alive-request from dest with "COMM_IS_REVOKED" => return error and do not try to receive alive-message again
		 */
		if(simft::Sim_FT_Check_comm_revoked(f_comm)){
