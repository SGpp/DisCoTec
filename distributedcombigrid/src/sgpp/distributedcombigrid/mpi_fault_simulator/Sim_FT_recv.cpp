...
int simft::Sim_FT_MPI_Recv(void *buf, int count, MPI_Datatype type, int source,
             int tag, simft::Sim_FT_MPI_Comm f_comm, simft::Sim_FT_MPI_Status *status)
{
    ...
    if ( Alive_Status_Recv == I_AM_ALIVE || Alive_Status_Recv == I_AM_ALIVE_NB ) {
    	...
    	if( status != SIM_FT_MPI_STATUS_IGNORE ){//if the status object does not exist, we aren't allowed to access it(!!!)
