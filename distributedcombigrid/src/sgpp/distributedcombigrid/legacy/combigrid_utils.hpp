...
/**
 * Utility function that checks if the precedence of template type _FGTp is
 *lower than that of
 ...
 *
 *
 * Use Case 1: check_type_precedence<float,double>(); --> should pass without
 *exception (as _FGTp <= _CFTp)
 * Use Case 2: check_type_precedence<float,float>(); --> should pass without
 *exception (as _FGTp = _CFTp)
 * Use Case 3: check_type_precedence<double,float>(); --> should throw an
 *exception (as _FGTp > _CFTp )
 *
 */
...
// vector declarations only for internal use , to make the interface more
// simpler
