...
GeneTask::GeneTask(DimType dim, const LevelVector& l, std::vector<BoundaryType>& boundary,
                   real coeff, LoadModel* loadModel, std::string& path, real dt, real combitime,
                   size_t nsteps, real shat, real lx, int ky0_ind, std::vector<int> p,
                   FaultCriterion* faultCrit, IndexType numSpecies, bool GENE_Global,
                   bool GENE_Linear)
...
//    }
    ...
    for( DimType d=0; d<dfgVector_[species]->getDimension(); ++d ){
      ...
      if( coords[d] == p[d] - 1 && ( d==1 || d == 2 || d == 3 || (d == 4 && !_GENE_Linear)/* ||d==5*/ ) ){ //y (only non-linear cases),x,z,v,w at upper border of domain (one additional point)
        ...
        else{ //lower border or in the middle of domain (no additional boundary point) or x
