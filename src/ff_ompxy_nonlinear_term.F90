...
module ff_ompxy_nonlinear_term_mod
  ...
  type, public, extends(ff_nonlinear_term_t) :: ff_ompxy_nonlinear_term_t
   contains
  ...
  end type ff_ompxy_nonlinear_term_t
...
contains
  ...
  function getThisType(this)
    class(ff_ompxy_nonlinear_term_t) :: this
    character(MAX_TYPENAME_LENGTH) :: getThisType

    getThisType = "ff_ompxy_nonlinear_term_t"
...
!--------------------------------------------------------------------------------------
  ...
  subroutine calc_nonlinearity_ff(this,p_g,p_emfields,p_barchi,p_rhs,stage,lb1,lb2)
    ...
    integer :: omp_get_thread_num,my_thread
    logical:: first
    ...
    !$OMP shared(sk,sl,sm,sn,lb1,lb2,li1,li2,hkx,lkx,lj1,lj2,kx_offset,li2da,this) &
    !$OMP shared(n_fields,first,pi1,pj2,equil_par_curr) &
    !$OMP shared(p_barchi,kx,ky,p_emfields,jfac,vTvpar,I1_factor,mu_tjqj,p_g,p_rhs) &
    !$OMP private(k,l,m,n,klmn,j) &
    !$OMP private(ve_x,ve_y,val,dG_x,dG_y,nonlin_real,nonlin)
    !$OMP DO
    do klmn=lb1,lb2
       ...
       if (associated(p_barchi)) then
          ! ---- compute the ExB velocity components as dchi/dx dchi/dy ----
          ...
          ! ---- first compute chi out of the fields ----
          ! ---- then compute ve_x,ve_y ----
       ...
       Call nl_to_direct_xy(ve_x,dG_x,0,nonlin_real,first)
       !write(*,"(2I3,A,ES17.10)") mype,my_thread,", nonlin_real 1 = ",sqrt(sum(nonlin_real**2))
       Call nl_to_direct_xy(ve_y,dG_y,-1,nonlin_real,first) 
    ...
    !$OMP END PARALLEL
    if (first) then
       ve_max(1) = max(ve_max(1),ve_max_omp_x)
       ve_max(2) = max(ve_max(2),ve_max_omp_y)
       !write(*,"(I3,A,2ES10.3)") mype,"ve_max = ",ve_max(1),ve_max(2)
    end if

  end subroutine calc_nonlinearity_ff

  Subroutine nl_to_direct_xy(inarr1,inarr2,was, outarr,first)
    ...
    if(fourier2d) then
       call nl_to_direct_xy_2d(inarr1,rearr1) ! ve_x/y
       call nl_to_direct_xy_2d(inarr2,rearr2) ! dG_x/y
    else
       call nl_to_direct_xy_1d(inarr1,rearr1) ! ve_x/y
       call nl_to_direct_xy_1d(inarr2,rearr2) ! dG_x/y
    end if
    ...
    if (first) then
       if (was.eq.0) then
          ...
          ve_max_omp_x=max(ve_max_omp_x,ve_x_max_loc)
          ...
          ve_max_omp_y=max(ve_max_omp_y,ve_y_max_loc)
...
end module ff_ompxy_nonlinear_term_mod
