...
int simft::Sim_FT_MPI_Irecv(void *buf, int count, MPI_Datatype datatype, int source, int tag,
	...
	//an alive-message can only be sent to a specific process. If source is MPI_ANY_SOURCE, send alive-message in MPI_TEST
	if(source != MPI_ANY_SOURCE){
		MPI_Isend(&Alive_Status_Send, 1, MPI_CHAR, source, tag, f_comm->c_comm_copy_p2p, &Send_Request);
