 ...
 */
void CombiMinMaxScheme::createClassicalThirdLevelCombischeme(unsigned int systemNumber,
                                                             std::vector<bool> boundary,
                                                             std::vector<LevelVector>& commonSubspaces) {
  // create classical scheme first
  createClassicalCombischeme();

  // split scheme
  std::vector<std::vector<LevelVector>> splittedScheme;
  std::vector<std::vector<combigrid::real>> splittedCoeffs;
  size_t numSystems = 2;
  splitScheme(splittedScheme, splittedCoeffs, numSystems);

  // assign half to system
  combiSpaces_  = splittedScheme[systemNumber];
  coefficients_ = splittedCoeffs[systemNumber];

  commonSubspaces = computeCommonSubspaces(splittedScheme, boundary);
}
 ...
 * third level combination.
 */
void CombiMinMaxScheme::createAdaptiveThirdLevelCombischeme(unsigned int systemNumber,
                                                            std::vector<bool> boundary,
                                                            std::vector<LevelVector>& commonSubspaces) {
  // create adaptive scheme first
  createAdaptiveCombischeme();
 ...
 */
void CombiMinMaxScheme::splitScheme(std::vector<std::vector<LevelVector>>& splittedScheme,
                                    std::vector<std::vector<real>>& splittedCoeffs,
                                    size_t numSystems){
  std::vector<LevelVector>& fullScheme = combiSpaces_;
  std::vector<real>& fullSchemeCoeffs = coefficients_;
  ...
  splittedScheme.reserve(numSystems);
  splittedCoeffs.reserve(numSystems);
  splittedScheme.push_back(std::move(lowerHalf));
  splittedScheme.push_back(std::move(upperHalf));
  splittedCoeffs.push_back(std::move(lowerCoeffs));
  splittedCoeffs.push_back(std::move(upperCoeffs));
}
 ...
 */
std::vector<LevelVector> CombiMinMaxScheme::computeCommonSubspaces(const std::vector<std::vector<LevelVector>>& splittedScheme, const std::vector<bool>& boundary){
  // compute common subspaces:
  // therefore we compute the component wise maximum level which is contained
  // in all sets
  long longMax = 2147483647;
  LevelVector maxLevel(dim_, longMax);
  for (size_t d = 0; d < dim_; d++) {
    for (const auto& levels : splittedScheme) {
      long dimMax = 0;
  ...
  std::vector<LevelVector> commonSubspaces;
  SGrid<real> sg(dim_, maxLevel, maxLevel, boundary);
