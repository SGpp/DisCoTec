...
int simft::Sim_FT_MPI_Comm_free(simft::Sim_FT_MPI_Comm *f_comm){
	//MPI_Comm_free is a collective function according to the standard, so we use our dead processes functions.

	//first sync to make sure that in case of a revoked comm all processes switched to the revoked tag
	...
	if((*f_comm)->comm_rank == (*f_comm)->Root_Rank){
		...
		if((*f_comm)->CurrentMaxNBC_Rank != (*f_comm)->comm_rank){
			...
			MPI_Isend(0, 0, MPI_INT, (*f_comm)->CurrentMaxNBC_Rank, SIM_FT_NBC_REQUEST_TAG, (*f_comm)->c_comm_copy, &NBC_Request);
			MPI_Request_free(&NBC_Request);

			MPI_Status RStatus;
			int PFlag = 0;
			while(PFlag == 0){
				MPI_Iprobe((*f_comm)->CurrentMaxNBC_Rank, SIM_FT_NBC_RESPONSE_TAG, (*f_comm)->c_comm_copy, &PFlag, &RStatus);
			...
			MPI_Recv(&(*f_comm)->NBC_Vector_Send.front(), Count, MPI_INT, (*f_comm)->CurrentMaxNBC_Rank, SIM_FT_NBC_RESPONSE_TAG, (*f_comm)->c_comm_copy, MPI_STATUS_IGNORE);
			// ####################
		}else{
	...
	if( 0 == Ret){ //if Ret != 0, we have a problem!
		...
		simft::Sim_FT_Current_Active_Communicators.erase(*f_comm); //remove freed communicator from active comm set
