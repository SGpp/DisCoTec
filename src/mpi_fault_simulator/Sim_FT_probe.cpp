...
int simft::Sim_FT_MPI_Probe(int source, int tag, simft::Sim_FT_MPI_Comm f_comm,
  ...
  MPI_Isend(&Probe_Req, 1, MPI_CHAR, source, tag, f_comm->c_comm_copy_p2p,
  ...
  // the sending process sends the information, how big the sending message is via a special probe
  // communicator
  int ProbeCount = 0;
  MPI_Recv(&ProbeCount, 1, MPI_INT, ProbeStatus.MPI_SOURCE, ProbeStatus.MPI_TAG,
           f_comm->c_comm_copy_probe, &probeRecvStatus);

  /* Normally we would store the count inside the status object, but on the HLRS HazelHen
   * implementation of MPI, count does not exist,
