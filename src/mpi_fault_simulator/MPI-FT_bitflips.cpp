...
void simft::Sim_FT_Manipulate_bits(int* IntArray, int ArraySize, double p) {
  ...
  // given the probability p, generate the amount of bits to be flipped randomly using the binomial
  // distribution
  ...
  if (BitsToFlip > 0) {
    ...
    // make sure, the first "BitsToFlip" entries of our vector are randomly selected from the whole
    // vector
    for (int i = 0; i < BitsToFlip; i++) {
       ...
       * Note: alternatively instead of flip it could be possible to always set a defective bit to 1
       * or always 0 or select it randomly.
      */
