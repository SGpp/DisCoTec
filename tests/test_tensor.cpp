...
BOOST_AUTO_TEST_CASE(test_6d) {
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(t.getExtents().begin(), t.getExtents().end(), extents.begin(),
                                extents.end());
  double increasingValue = 0.0;
  // make sure that Fortran ordering is the default
  for (IndexType i = 0U; i < t.getExtents()[5]; ++i) {
    for (IndexType j = 0U; j < t.getExtents()[4]; ++j) {
      for (IndexType k = 0U; k < t.getExtents()[3]; ++k) {
        for (IndexType l = 0U; l < t.getExtents()[2]; ++l) {
          for (IndexType m = 0U; m < t.getExtents()[1]; ++m) {
            for (IndexType n = 0U; n < t.getExtents()[0]; ++n) {
