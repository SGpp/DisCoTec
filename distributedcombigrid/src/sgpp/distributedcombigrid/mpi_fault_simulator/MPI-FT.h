...
//how long does the root wait until it sends a NBC broadcast to inform comm about a dead process (including non-blocking count)
#define SIM_FT_WAIT_NBC_BCAST 5

//#define DISABLE_NBC //if defined, do not use non-blocking collectives. Required for MPI implementations not supporting non-blocking collectives.
 ...
 * that are required to be able to complete them. Has to be extended if more non-blocking ops are to be used! */
...
struct Sim_FT_MPI_Comm_struct {
    ...
    MPI_Comm                c_comm_copy_coll = MPI_COMM_NULL;		//copy used send/recv messages in the layer (used for collectives)
    MPI_Comm                c_comm_copy_p2p = MPI_COMM_NULL;		//copy used send/recv messages in the layer (used for p2p)
    MPI_Comm                c_comm_copy_probe = MPI_COMM_NULL;		//copy used send/recv messages in the layer (used for probe)
    bool                    comm_revoked = false;  //set true if the communicator is revoked
    bool                    manual_revoke_message = false; //indicates if a revoke message has to be manually forwared (by using Sim_FT_Only_propagate_comm_revoked)
    ...
    std::vector<int>		Bcast_Successors;     //in the custom broadcast used to identify the successor nodes of the custom tree topology
    ...
    std::set<int>			root_recent_dead_set;	//for nbc broadcast. attach recent dead processes not broadcasted yet
    std::set<int>			dead_set;               //used to quickly get the information if a given process is already detected as dead

    bool err_return = false;						//by setting errhandler to MPI_ERRS_RETURN, this will be set to true

    //variable needed for MPI_Comm_failure_ack; set to true if current failed processes are acknowledged
    ...
    //used to store the serialized updated last non-blocking collectives (received from another process), will be sent via broadcast
    ...
    bool CurrentMinNBC_Modified = false; //set to true if CurrentMinNBC changed after last NBC broadcast (need new broadcast!)
...
//	 * ############################################*/
//    MPI_Request Revoke_Request; //If comm is to be revoked, broadcast is executed by root. Request is only used once at comm creation or revoke initiation (root).
////TODO
...
//Use a customized Request-Struct be able to get the communicator, the request type and more just from the request
struct Sim_FT_MPI_Request_struct {
...
//custom non-blocking broadcast used to send background informations and inside Check_dead_processes (root version)
bool Sim_FT_Custom_Ibcast_Root(std::vector<int> *buf_out, std::vector<int> *buf_copy, int count, MPI_Datatype datatype, int tag, MPI_Comm comm, Sim_FT_MPI_Comm f_comm);
//custom non-blocking broadcast used to send background informations and inside Check_dead_processes (non-root version)
bool Sim_FT_Custom_Ibcast_NRoot(std::vector<int> *buf_out, std::vector<int> *buf_copy, int *count, MPI_Datatype datatype, int tag, MPI_Comm comm, Sim_FT_MPI_Comm f_comm);

//custom reduce used inside Check_dead_processes. NextOp defines
int Sim_FT_Custom_Reduce(int * Send_Vector, Sim_FT_MPI_Comm f_comm, NextOp option = NextOp::Default, bool Revoked = false);
...
/* Calculates the successor of node_id in our simple tree topology given root, current node id (0,...,tree_size -1) and max. Successor_Count.
 * Successor_Count 2 would generate a binary tree. */
std::vector<int> Bcast_Get_Successors(int root_node, int node_id, int tree_size, int Successor_Count = -1);
/* Calculates the predecessor of node_id in our simple tree topology given root, current node id (0,...,tree_size -1) and max. Successor_Count.
 * Successor_Count 2 would generate a binary tree. */
int Bcast_Get_Predecessor(int root_node, int node_id, int tree_size, int Successor_Count = -1);

//helper function create the custom tree topology (makes the id behave like Z/nZ with n = tree_size)
...
/*
 * If we have a dead process and LastNBC_bcast is some time ago, broadcast the current lowest count of
 * active non-blocking collectives from all dead processes known to root and a list of all recently new dead processes.
 *
 * If an active process waits for a non-blocking collective to finish, possibly a dead process never contributed
 * to the collective and the process waits infinitely. After receiving a NBC broadcast, the process is
 * knows about it and returns the wait with a failure.
 */
void Sim_FT_NBC_bcast_root(Sim_FT_MPI_Comm f_comm);
//background broadcast containing count of recent non-blocking collectives and list of recently killed processes (non-root version)
...
//perform all important background operations for a specific communicator (special vesion for dead processes)
...
//if a process wants the communicator to be revoked, it calls this function and a message is sent to root
...
//decides, when a process should get killed (customizable, for example randomly or after a certain amount of MPI calls)
 ...
 * */
void Sim_FT_Perform_nb_operations(std::vector<Sim_FT_Istats> * NB_ops_to_perform, Sim_FT_MPI_Comm f_comm);
 ...
 * */
void Sim_FT_Complete_nb_operations(std::vector< simft::Sim_FT_Istats > *Deserialized_NBC_Vector, Sim_FT_MPI_Comm f_comm);
...
int Sim_FT_MPI_Allreduce(void *sendbuf, void *recvbuf, int count,
                  MPI_Datatype datatype, MPI_Op op, Sim_FT_MPI_Comm f_comm);
int Sim_FT_MPI_Barrier(Sim_FT_MPI_Comm f_comm);
int Sim_FT_MPI_Bcast(void *buffer, int count, MPI_Datatype datatype, int root, Sim_FT_MPI_Comm f_comm);
int Sim_FT_MPI_Cart_coords(Sim_FT_MPI_Comm f_comm, int rank, int maxdims, int coords[]);
int Sim_FT_MPI_Cart_create(Sim_FT_MPI_Comm comm_old, int ndims, int dims[],
                    int periods[], int reorder, Sim_FT_MPI_Comm *comm_cart);
...
int Sim_FT_MPI_Iallreduce(void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype,
   MPI_Op op, Sim_FT_MPI_Comm f_comm, Sim_FT_MPI_Request *request);
int Sim_FT_MPI_Ibarrier(Sim_FT_MPI_Comm f_comm, Sim_FT_MPI_Request *request);
int Sim_FT_MPI_Ibcast(void *buffer, int count, MPI_Datatype datatype, int root, Sim_FT_MPI_Comm f_comm,
		Sim_FT_MPI_Request *request);
#endif
...
int Sim_FT_MPI_Recv(void *buf, int count, MPI_Datatype type, int source,
             int tag, Sim_FT_MPI_Comm comm, Sim_FT_MPI_Status *status);
int Sim_FT_MPI_Reduce(void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype,
               MPI_Op op, int root, Sim_FT_MPI_Comm f_comm);
int Sim_FT_MPI_Request_free(Sim_FT_MPI_Request *request);
int Sim_FT_MPI_Send(void *buf, int count, MPI_Datatype type, int dest,
             int tag, Sim_FT_MPI_Comm comm) ;
...
//Function needed to be able to simulate bit errors; special case for integer arrays. p specifies the flip probability of a bit
void Sim_FT_Manipulate_bits( int * BitArray, int ArraySize, double p );

//use the c++ internal function to calculate for a binomial distribution the amount of successful events (in our case: bit flips)
