...
module df_ompxy_nonlinear_term_mod
  ...
  type, public, extends(df_nonlinear_term_t) :: df_ompxy_nonlinear_term_t
   contains
     procedure :: calc => calc_ompxy_nonlinearity_df
     procedure :: getType => getThisType
     procedure :: allocate_arrays => allocate_arrays_omp
  end type df_ompxy_nonlinear_term_t
...
contains
  ...
  function getThisType(this)
    class(df_ompxy_nonlinear_term_t) :: this
    character(len=MAX_TYPENAME_LENGTH) :: getThisType

    getThisType = "df_ompxy_nonlinear_term_t"
  ...
  Subroutine calc_ompxy_nonlinearity_df(this,gy_chi,g_block,vexy,dgdxy,localrhs,first)
    class(df_ompxy_nonlinear_term_t) :: this
    ...
    !DEC$ ATTRIBUTES ALIGN:64 :: vexy_re, dgdxy_re, nonlin, nonlin1_re
    !Real:: ve_x_max_loc, ve_y_max_loc
    Integer :: i,j, klmn
    ...
    do klmn=1,this%lbg0
       !$OMP PARALLEL &
       !$OMP default(none) &
       !$OMP private(i,j) shared(li0da,n_procs_y,ly0da,klmn) &
       !$OMP shared(nonlin1_re,vexy_re,dgdxy_re)
       PERFON('nl_kernl')
       !$OMP DO
       do i=0,li0da/n_procs_y-1
          do j=0,ly0da-1
             nonlin1_re(j,i) = -vexy_re(j,i,1,klmn)*dgdxy_re(j,i,2,klmn) &
                  & + vexy_re(j,i,2,klmn)*dgdxy_re(j,i,1,klmn)
          end do
       end do
       !$OMP END DO
       PERFOFF
  ...
  End Subroutine calc_ompxy_nonlinearity_df
...
#endif
end module df_ompxy_nonlinear_term_mod
