...
SOURCE_PATH=${SOURCE_PATH:=/hppfs/work/pn36xu/di39qun2/3-systems/measurements/3-systems_8x2048/}
DSG_NUM=${DSG_NUM:=1}
SOURCE_FILE=${SOURCE_FILE:=${SOURCE_PATH}/dsg_${DSG_NUM}_step*_0}

TARGET_PATH_JUEWLS=${TARGET_PATH_JUEWLS:=//p/scratch/widediscotecjsc/3-systems/measurements/3-systems_3x2048/}
TARGET_URL_JUEWLS=https://uftp.fz-juelich.de:9112/UFTP_Auth/rest/auth/JUDAC
TARGET_USER_JUEWLS=${TARGET_USER_JUEWLS:=pollinger2}
IDENTITYFILE_JUEWLS=${IDENTITYFILE_JUEWLS:=~/.uftp/id_uftp_to_jsc}

TARGET_PATH_HLRS=${TARGET_PATH_HLRS:=//lustre/hpe/ws11/ws11.1/ws/ipvpolli-discotec_weak_scaling/3-systems/measurements/3-systems_16x2048/}
...
HOST_NR_NG=$(( $(hostname | tr -dc '0-9') - 5 ))
NUM_HOSTS=3
...
do
    for part in $LOCAL_SEQUENCE; do
      ...
      if test -f $TOKEN_TRANSFER_FORWARD; then
          ...
          {
            starttime=`date +%s`
            echo "$(date +%T) start copying $(basename $SOURCE_FILE_INSTANCE) to JUEWLS as copie $copie / $NUM_LOCAL_FILES"
            # copy the compressed file
            uftp cp -n ${STREAMS} -t ${THREADS_PER_PROC} -i ${IDENTITYFILE_JUEWLS} -u ${TARGET_USER_JUEWLS} ${SOURCE_FILE_INSTANCE} ${TARGET_URL_JUEWLS}:${TARGET_PATH_JUEWLS}
            # generate TOKEN_TRANSFER_FORWARD on remote system
            uftp cp -i ${IDENTITYFILE_JUEWLS} -u ${TARGET_USER_JUEWLS} -B0 /dev/zero ${TARGET_URL_JUEWLS}:${TARGET_PATH_JUEWLS}/$(basename ${TOKEN_TRANSFER_FORWARD})
            endtime=`date +%s`
            echo $(date +%T) copied $(basename $SOURCE_FILE_INSTANCE) in $((endtime-starttime)) seconds to JUWELS.
          } &
          {
            starttime=`date +%s`
