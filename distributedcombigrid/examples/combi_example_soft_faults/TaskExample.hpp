...
//    assert(dfg_ == NULL);
  ...
  void run(CommunicatorType lcomm) {
    ...
    if ( failNow(localRank) && initialized_ ){
      std::cout<<"Task "<< id_<<" failed at iteration "<<combiStep_<<std::endl;
      std::cout<<"This local rank: "<<localRank<<std::endl;
      IndexType sdcGLI = dfg_->getGlobalLinearIndex(faultsInfo_.sdcIndex_);
      IndexType sdcLinearIndex = dfg_->getLocalLinearIndex(sdcGLI);

      std::cout<<"Old function value: "<<dfg_->getData()[sdcLinearIndex]<<std::endl;
      dfg_->getData()[sdcLinearIndex] *= faultsInfo_.sdcMag_;
      std::cout<<"New function value: "<<dfg_->getData()[sdcLinearIndex]<<std::endl;
      ...
      std::cout<<"SDC injected in subspace "<<sdcSub<<std::endl;
      dfg_->getCoordsLocal( sdcLinearIndex, coords );
      std::cout<<"Coordinate: ";
      for (auto coord : coords)
        std::cout<<coord<<" ";
      std::cout<<std::endl<<"Local index: "<<sdcLinearIndex<<std::endl;
    }
