...
class TaskAdvFDM : public combigrid::Task<> {
 ...
 public:
  ...
  void init(CommunicatorType lcomm,
    ...
    dfg_.emplace(DIM, getLevelVector(), lcomm, getBoundary(), p);
  ...
  void run(CommunicatorType lcomm) {
    ...
    IndexType l0 = dfg.globalNumPointsInDimension(0);
    IndexType l1 = dfg.globalNumPointsInDimension(1);
    ...
    for (size_t i = 0; i < nsteps_; ++i) {
      ...
      for (IndexType li = 0; li < dfg.getNrElements(); ++li) {
        ...
        CombiDataType dphi0 = (phi_[li] - phi_[lwi]) / h0;
        CombiDataType dphi1 = (phi_[li] - phi_[lsi]) / h1;

        CombiDataType u_dot_dphi = u0 * dphi0 + u1 * dphi1;
        dfg.getData()[li] = phi_[li] - u_dot_dphi * dt_;
      }
    }
  ...
  void setStepsTotal(size_t stepsTotal) { stepsTotal_ = stepsTotal; }
    ...
    for (size_t i = 1; i < ncombi; ++i) {
      if (!success) {
        ...
        if (failedRecovery) {
          ...
          // if process groups could be restored reinitialize restored process group (keep the
          // original tasks)
