...
int simft::Sim_FT_MPI_Cart_create(simft::Sim_FT_MPI_Comm comm_old, int ndims, int dims[],
	...
	int Ret = MPI_Cart_create(comm_old->c_comm, ndims, dims,
            periods, reorder, &(*comm_cart)->c_comm);

	if(MPI_SUCCESS == Ret){
		simft::Sim_FT_Initialize_new_comm((comm_cart), true); //TODO: if "reroot" implemented, set to false
	}else{
		delete (*comm_cart);
		(*comm_cart) = nullptr;
	}
