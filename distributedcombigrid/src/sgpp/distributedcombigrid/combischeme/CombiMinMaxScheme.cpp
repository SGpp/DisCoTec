...
void CombiMinMaxScheme::makeFaultTolerant() {
  ...
  for (size_t i = 0; i < levels_.size(); ++i) {
    ...
    for (LevelType p = LevelType(effDim_);
        p < LevelType(effDim_) + extraDiags; ++p) {
      if (l >= lmin_ && sum(l) == n_ - p) {
        combiSpaces_.push_back(l);
...
void CombiMinMaxScheme::createLevelsRec(DimType dim, LevelType n, DimType d,
    LevelVector &l, const LevelVector& lmax) {
...
void CombiMinMaxScheme::computeCombiCoeffsAdaptive(){
  ...
  for(size_t i = 0; i < combiSpaces_.size(); i++){
    ...
    if (coeff == 0)
      combiSpaces_[i] = LevelVector(dim_,0);
    coefficients_.push_back(coeff);
  }
  // Delete combiSpaces_ and coefficients_ for which the coeff is zero
  combiSpaces_.erase(std::remove(combiSpaces_.begin(),combiSpaces_.end(),LevelVector(dim_,0)),combiSpaces_.end());
  coefficients_.erase(std::remove(coefficients_.begin(),coefficients_.end(),0),coefficients_.end());
}
