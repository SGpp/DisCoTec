...
protected:
	...
	StaticCombiScheme(const LevelVector& lmin, const LevelVector& lmax, bool faultTolerant) :
		 ...
		 * grids that make up the combi scheme.
		 */

		/**
		 * Returns the maximum possible sum of the levels of the grids
		 * that are contained in this scheme.
		 * If lmin = (a, a, ...) and lmax (b, b, b, ...) (ignoring dummy dimensions)
		 * then the "corners" of the scheme are at (b, a, a, ...), (a, b, a, ...),
		 * (a, a, b, ...), ..., meaning that their index sum is equivalent to the max
		 * allowed index sum.
		 */
		LevelType getMaxLevelSum();

		/**
		 * The grid are generated (recursively) starting at the back.
		 */
		void createLevelsRec(){
			createLevelsRec(getMaxLevelSum(), dim() - 1, LevelVector(dim(), 0));
		}

		void createLevelsRec(LevelType currMaxSum, size_t currentIndex, LevelVector l);
		...
		/*
		 * Returns true if the grids lies outside of the border of this scheme
		 * as indicated by lmin (equiv. to at least one index being smaller than
		 * the corresponding index in lmin)
		 */
